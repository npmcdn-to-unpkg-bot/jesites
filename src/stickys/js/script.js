...
if(window.openDatabase) {
    ...
    alert("Failed to open database. MAke sure your browser support HTML5");
...
Note.prototype = {
    ...
    save: function() {
        ...
        var note = this;
        this.text = this.editField.innerHTML;
        db.transaction(function(tx) {
    ...
    db.transaction(function(tx){
        tx.executeSql("SELECT id,note,timestamp,left,top, zIndex FROM MyStickys", [], function(tx,result) {
            for(var i=0; i< result.rows.length;i++) {
                ...
                //note.editField.innerHTML = note.text;
...
function modifiedString(date) {
    return "Stycky Last Modified: " + date.getFullYear() + "-" + (date.getMonth()+1) + "-" + date.getHours() + ":" + date.getMinutes() + ":" + date.getSeconds();
